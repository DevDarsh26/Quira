...
class quiraPipeline:
    ...
    def __init__(
        ...
        self.enable_agentic_routing = enable_agentic_routing

        # Resolve Vector Store
        if edge_mode:
            if edge_store == "duckdb":
                from quira.providers.vector.embedded_store import DuckDBStore
                self.vector_store = DuckDBStore()
            else:
                from quira.providers.vector.embedded_store import SQLiteVecStore
        ...
        if self.enable_graph_rag:
            # If the selected vector store is Neo4j, we can reuse it as our GraphStore!
            if isinstance(getattr(self, 'vector_store', None), __import__('quira.providers.vector.neo4j_store', fromlist=['Neo4jStore']).Neo4jStore) if 'Neo4jStore' in [cls.__name__ for cls in getattr(self, 'vector_store', object).__class__.__mro__] else False:
                self.graph_store = self.vector_store
            else:
                try:
                    from quira.providers.vector.neo4j_store import Neo4jStore
                    if isinstance(self.vector_store, Neo4jStore):
                        self.graph_store = self.vector_store
                    else:
        ...
        self.ingestor = DocumentIngestor(
            ...
            extraction_llm=self._compression_llm or self.llm
        )
    ...
    async def process_submission(
        ...
        # === AGENTIC ROUTING ===
        if self.enable_agentic_routing:
            from quira.modules.routing import is_conversational_query, get_canned_conversational_response
            if is_conversational_query(final_query):
                canned = get_canned_conversational_response(final_query)
                if canned:
                    self.last_run_metrics = {"agentic_routing_hit": True, "tokens_saved": 0}
                    return canned
                else:
                    ans = await self.llm.complete(prompt=final_query, system_prompt="You are a friendly AI assistant.")
                    self.last_run_metrics = {"agentic_routing_hit": True, "tokens_saved": 0}
                    return ans
        ...
        if self.enable_graph_rag and getattr(self, 'graph_store', None):
            graph_neighbors = await self.graph_store.get_neighbors(final_query, max_hops=1)
            for neighbor in graph_neighbors:
                cid = f"graph_{hash(neighbor['subject'] + neighbor['relation'] + neighbor['object'])}"
                if cid not in existing_ids:
                    text = f"{neighbor['subject']} {neighbor['relation']} {neighbor['object']}"
                    diff_pool.append({
                        "id": cid,
                        "text": text,
                        "embedding": None, 
                        "hit_count": 1,
                        "payload": {"source": "graph_rag"}
                    })
                    existing_ids.add(cid)
        ...
        if self.enable_agentic_routing:
            ...
            if is_conversational_query(final_query):
                ...
                if canned:
                    ...
                    async for chunk in self.llm.stream(prompt=final_query, system_prompt="You are a friendly AI assistant."):
                        yield chunk
                    return
