...
class DocumentIngestor:
    ...
    def __init__(
        ...
        extraction_llm: Optional[LLMProvider] = None
    ):
...
Return the result as a JSON list of arrays, like:
...
"""
        try:
            response = await self.extraction_llm.complete(prompt=prompt)
            import json, re
            ...
            if match:
                ...
                for item in raw_list:
                    if isinstance(item, list) and len(item) == 3:
                        triplets.append((str(item[0]), str(item[1]), str(item[2]), {"source": "ingestion"}))
                return triplets
    ...
    async def ingest_text(self, user_id: str, text: str, chunk_size: int = 1000, overlap: int = 200) -> int:
        ...
        try:
            ...
            # GraphRAG extraction
            if self.graph_store and self.extraction_llm:
                logger.info(f"User {user_id}: Extracting Knowledge Graph triplets...")
                for p in points:
                    triplets = await self._extract_triplets(p.payload["text"])
                    if triplets:
                        await self.graph_store.add_triplets(triplets)
                logger.info(f"User {user_id}: Successfully extracted and stored triplets.")
